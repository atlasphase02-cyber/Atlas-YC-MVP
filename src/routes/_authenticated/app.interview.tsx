...
const KICKOFF =
  "Hi, I'm Atlas. I'll ask a few short questions to capture this claim. Let's start with what happened — where was the damage first noticed, and when?";
...
function Page() {
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const seed: InterviewTurn[] = [
        { role: "assistant", content: KICKOFF, at: new Date().toISOString() },
      ];
      const { data, error } = await db
        .from("interviews")
        .insert({
            ...
            <div className="space-y-1 max-h-[520px] overflow-y-auto">
              ...
              {!list.isLoading && (list.data?.length ?? 0) === 0 && (
                <EmptyState
                  ...
                  hint="Start one to walk a customer through intake."
                />
              )}
              {list.data?.map((iv) => (
                <button
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Badge
                      variant={iv.status === "completed" ? "secondary" : "default"}
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {new Date(iv.updated_at).toLocaleString()}
                  </p>
                </button>
              ))}
            </div>
            ...
            <Card className="panel-atlas border-0">
              <CardContent className="p-10 text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1 max-w-md mx-auto">
                  Start a new interview or pick one from the left. Every turn is saved automatically
                  and Atlas can generate a summary + action items when you finish.
                </p>
              </CardContent>
            </Card>
          )}
          ...
          <div className="flex items-center justify-between gap-2 px-4 py-3 border-b border-border">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-muted-foreground">
                  {iv.transcript.length} turns · {iv.status}
                </p>
              </div>
            </div>
            <Button
        ...
        <Card className="panel-atlas border-0">
          <CardContent className="p-4 space-y-3">
            <Button
              ...
              disabled={generating || iv.transcript.length < 2}
            >
              {generating ? (
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              ) : (
