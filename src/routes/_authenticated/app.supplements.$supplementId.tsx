...
export const Route = createFileRoute("/_authenticated/app/supplements/$supplementId")({
  component: Page,
});
...
function Page() {
  ...
  const total = (items.data ?? []).reduce(
    (s, i) => s + Math.round(Number(i.quantity) * i.unit_price_cents),
    0,
  );
  ...
  const delItem = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await db.from("supplement_items").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["supplement-items", supplementId] }),
  });
    ...
    mutationFn: async () => {
      ...
      if (data.items.length > 0) {
        const rows = data.items.map((it) => ({
          ...
          ai_suggested: true,
          ai_confidence: Math.round(it.confidence),
          ai_reason: it.reason,
        }));
  ...
  if (supp.error || !supp.data)
    return (
      <AppShell title="Supplement">
        <ErrorState
          message={(supp.error as Error)?.message ?? "Not found"}
          onRetry={() => supp.refetch()}
        />
      </AppShell>
    );
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 sm:flex sm:flex-wrap sm:justify-between">
          <Button asChild variant="ghost" size="sm" className="justify-self-start">
            <Link to="/app/supplements">
              <ArrowLeft className="mr-2 h-4 w-4" aria-hidden="true" /> Supplements
            </Link>
          </Button>
          <div className="col-span-2 flex flex-wrap items-center gap-2 sm:col-span-1">
            <Select
              value={s.status}
              onValueChange={(v) => updateSupp.mutate({ status: v as SupplementStatus })}
            >
              <SelectTrigger className="w-full sm:w-40" aria-label="Supplement status">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {SUPPLEMENT_STATUSES.map((x) => (
          ...
          <CardContent className="p-4 space-y-3">
            ...
            <div>
              ...
              <Textarea
                ...
                onBlur={(e) => updateSupp.mutate({ summary: e.target.value })}
              />
            </div>
            {s.ai_summary && (
        ...
        <Card className="panel-atlas border-0">
          <CardContent className="p-0">
            ...
            {items.isLoading ? (
              ...
              <div className="p-4">
                <EmptyState title="No line items" hint="Add scope items or click Draft with AI." />
              </div>
            ) : (
              <div className="overflow-x-auto">
                <div className="divide-y divide-border min-w-[640px]">
                  ...
                  <div className="grid grid-cols-12 gap-2 px-4 py-3 bg-white/5">
                    <div className="col-span-10 text-right text-sm text-muted-foreground">
                      Total
                    </div>
                    <div className="col-span-1 text-right font-mono text-atlas-signal">
                      {formatMoney(total)}
                    </div>
                    <div className="col-span-1"></div>
                  </div>
...
function ItemRow({
  ...
  useEffect(() => {
    ...
    setUnit((item.unit_price_cents / 100).toFixed(2));
    ...
    <div className="grid grid-cols-12 gap-2 px-4 py-2 items-center">
      ...
      <div className="col-span-2">
        ...
        <Input
          ...
          onBlur={() => onChange({ quantity: Number(qty) || 0 })}
        />
      </div>
      <div className="col-span-2">
        ...
        <Input
          ...
          onBlur={() => onChange({ unit_price_cents: Math.round(Number(unit) * 100) || 0 })}
        />
      </div>
      <div className="col-span-1 text-right font-mono text-sm">{formatMoney(total)}</div>
      <div className="col-span-1 text-right">
        <Button variant="ghost" size="icon" aria-label="Delete line item" onClick={onDelete}>
