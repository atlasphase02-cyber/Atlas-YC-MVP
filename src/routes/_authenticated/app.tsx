...
function DashboardPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      ...
      setName(
        (meta?.full_name || meta?.name || data.user?.email?.split("@")[0] || "there").split(" ")[0],
      );
    });
  }, []);
  ...
  const metrics = useQuery({
    ...
    queryFn: async () => {
      ...
      const suppPending = supplements.filter(
  ...
  return (
    <AppShell
      ...
      subtitle="Here's where the business stands right now."
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Metric
            ...
            value={metrics.data ? formatMoney(metrics.data.revenueAtRisk) : null}
            trend={`${metrics.data?.openClaims ?? 0} open claims`}
            tone="warning"
          ...
          <Metric
            icon={FileText}
            label="Open claims"
            value={metrics.data ? String(metrics.data.openClaims) : null}
          ...
          <Metric
            ...
            value={metrics.data ? String(metrics.data.suppCount) : null}
            trend={metrics.data ? `Est. ${formatMoney(metrics.data.suppValue)}` : ""}
            tone="signal"
            loading={metrics.isLoading}
          />
          <Metric
            icon={Users}
            label="Customers"
            value={metrics.data ? String(metrics.data.customers) : null}
            trend="Active in Atlas"
            tone="default"
            loading={metrics.isLoading}
          />
        </div>
                ...
                <div key={r.id} className="py-3 flex items-start justify-between gap-4">
                  ...
                  {r.action?.route ? (
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => navigate({ to: r.action!.route as "/app" })}
                    >
                      {r.action.label} <ArrowRight className="ml-1 h-3 w-3" />
                    </Button>
                  ) : r.action?.ask ? (
                    <Button
                      size="sm"
                      ...
                      onClick={() =>
                        window.dispatchEvent(
                          new CustomEvent("atlas:ask", { detail: { prompt: r.action!.ask } }),
                        )
                      }
                    >
                      {r.action.label}
          ...
          <Card className="panel-atlas border-0">
            ...
            <CardContent className="space-y-3">
              ...
              <Button
                ...
                onClick={() =>
                  window.dispatchEvent(
                    new CustomEvent("atlas:ask", { detail: { prompt: "Summarize my day" } }),
                  )
                }
              >
