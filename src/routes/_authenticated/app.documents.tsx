...
import {
  db, currentUserId, uploadFile, signedUrl, deleteFile,
  DOC_BUCKET, type AppDocument,
...
function Page() {
  ...
  return (
    <PageStub title="Documents" subtitle={`${data?.length ?? 0} files`} askPrompt="Find contracts missing customer signatures">
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-full sm:min-w-56">
          <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" aria-hidden="true" />
          <label htmlFor="doc-search" className="sr-only">Search files and tags</label>
          <Input id="doc-search" className="pl-8" placeholder="Search files & tags" value={query} onChange={(e) => setQuery(e.target.value)} />
        </div>
        <input ref={fileRef} type="file" multiple className="hidden" aria-hidden="true" tabIndex={-1}
        ...
        <Button size="sm" onClick={() => fileRef.current?.click()} disabled={upload.isPending}>
          <Upload className="mr-2 h-4 w-4" aria-hidden="true" /> {upload.isPending ? "Uploading..." : "Upload"}
        </Button>
      </div>
      ...
      {!isLoading && !error && folders.size > 0 && (
        <div className="flex flex-wrap gap-2" role="group" aria-label="Filter by folder">
          <Badge variant={folderFilter === null ? "default" : "secondary"} onClick={() => setFolderFilter(null)}
            className="cursor-pointer" role="button" tabIndex={0}
            onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setFolderFilter(null); } }}>All</Badge>
          {Array.from(folders.entries()).map(([f, n]) => (
            <Badge key={f} variant={folderFilter === f ? "default" : "secondary"}
              className="cursor-pointer" role="button" tabIndex={0}
              onClick={() => setFolderFilter(f)}
              onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setFolderFilter(f); } }}>
      ...
      {!isLoading && !error && filtered.length === 0 && (
        <EmptyState title={data?.length ? "No files match" : "No documents yet"}
          ...
          <div className="divide-y divide-border">
            {filtered.map((d) => (
              <div key={d.id} className="p-3 flex items-center justify-between gap-2 sm:gap-3">
                <button className="text-left min-w-0 flex-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 rounded" onClick={() => open(d)} aria-label={`Open ${d.name}`}>
                  <p className="text-sm truncate">{d.name}</p>
                  <p className="text-[11px] text-muted-foreground truncate">
                    {d.folder} · {d.mime_type ?? "file"} · {d.size_bytes ? Math.round(d.size_bytes / 1024) + " KB" : ""}
                  </p>
                </button>
                <Button variant="ghost" size="icon" onClick={() => open(d)} aria-label={`Download ${d.name}`}><Download className="h-4 w-4" aria-hidden="true" /></Button>
                <Button variant="ghost" size="icon" onClick={() => del.mutate(d)} aria-label={`Delete ${d.name}`}><Trash2 className="h-4 w-4" aria-hidden="true" /></Button>
              </div>
            ))}
          </div>
        </CardContent></Card>
