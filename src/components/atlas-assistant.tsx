...
import { Mic, MicOff, Send, X, Loader2, Volume2, VolumeX, Plus, MessageSquare, PanelLeft, Settings2, Pin, Archive } from "lucide-react";
...
function pickVoice(preferredName?: string | null): SpeechSynthesisVoice | null {
  ...
  return en.find((v) => /natural|neural|premium|studio/i.test(v.name))
...
export function AtlasAssistant() {
  ...
  async function persistMessage(conversationId: string, role: "user" | "assistant", content: string, parts?: unknown) {
    const uid = await currentUserId();
    if (!uid) return;
    await db.from("messages").insert({
      owner_id: uid, conversation_id: conversationId, role, content, parts: parts ?? null,
    });
    qc.invalidateQueries({ queryKey: ["conversations"] });
  }
  ...
  async function submitText(text: string) {
    ...
    try {
      ...
      if (hits.length) {
        semanticRef.current = "Relevant records from the user's workspace (retrieved by semantic search):\n" +
          hits.map((h) => `- [${h.entity_type}] ${h.label}${h.sub ? ` — ${h.sub}` : ""} (id: ${h.entity_id})`).join("\n");
      } else {
        semanticRef.current = "";
      }
    } catch { semanticRef.current = ""; }
    sendMessage({ text: trimmed });
  }

  function startListening() {
    const Ctor = getRecognitionCtor();
    if (!Ctor) { setListening(false); return; }
    const rec = new Ctor();
    rec.lang = "en-US";
        ...
        <div className="fixed bottom-24 right-3 left-3 sm:left-auto sm:right-6 z-40 sm:w-[440px] sm:max-w-[calc(100vw-2rem)] h-[min(600px,calc(100dvh-8rem))] panel-atlas rounded-2xl flex flex-col overflow-hidden animate-reveal">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-border">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-xs text-muted-foreground truncate">
                {busy ? (status === "submitted" ? "Thinking…" : "Responding…") : listening ? "Listening…" : "Ready"}
              </span>
            </div>
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="icon" onClick={() => setPanel(panel === "history" ? "chat" : "history")} className="h-8 w-8" aria-label="History">
                <PanelLeft className="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={() => void switchTo(null)} className="h-8 w-8" aria-label="New conversation">
                <Plus className="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={() => setPanel(panel === "settings" ? "chat" : "settings")} className="h-8 w-8" aria-label="Voice settings">
                <Settings2 className="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={() => { updatePrefs({ muted: !prefs.muted }); if (!prefs.muted) window.speechSynthesis?.cancel(); }} className="h-8 w-8">
          ...
          {panel === "history" && (
            <div className="flex-1 overflow-y-auto p-3 space-y-1">
              <p className="px-2 pb-2 text-[11px] uppercase tracking-widest text-muted-foreground">Recent conversations</p>
              {(conversations.data ?? []).length === 0 && (
                <p className="px-2 py-6 text-sm text-muted-foreground text-center">No conversations yet.</p>
              )}
              {(conversations.data ?? []).map((c) => (
                <div key={c.id} className={cn("group flex items-center gap-1 rounded-lg px-2 py-2 hover:bg-white/5", convId === c.id && "bg-primary/10")}>
                  <button className="flex-1 text-left min-w-0" onClick={() => void switchTo(c.id)}>
                    ...
                    <p className="text-[10px] text-muted-foreground pl-5">{new Date(c.last_message_at).toLocaleString()}</p>
                  </button>
                  <Button variant="ghost" size="icon" className="h-7 w-7 opacity-0 group-hover:opacity-100" onClick={() => void pinConv(c.id, !c.pinned)}>
                    <Pin className={cn("h-3.5 w-3.5", c.pinned && "fill-current")} />
                  </Button>
                  <Button variant="ghost" size="icon" className="h-7 w-7 opacity-0 group-hover:opacity-100" onClick={() => void archiveConv(c.id)}>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-5">
              ...
              <div>
                <label className="text-xs uppercase tracking-widest text-muted-foreground flex items-center justify-between">
                  Speaking rate <span className="font-mono text-foreground">{Number(prefs.rate).toFixed(2)}×</span>
                </label>
                <Slider className="mt-2" value={[Number(prefs.rate)]} min={0.5} max={2} step={0.05}
                  onValueChange={(v) => updatePrefs({ rate: v[0] })} />
              </div>
              <div>
                <label className="text-xs uppercase tracking-widest text-muted-foreground flex items-center justify-between">
                  Pitch <span className="font-mono text-foreground">{Number(prefs.pitch).toFixed(2)}</span>
                </label>
                <Slider className="mt-2" value={[Number(prefs.pitch)]} min={0.5} max={2} step={0.05}
                  onValueChange={(v) => updatePrefs({ pitch: v[0] })} />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm">Auto-send voice transcripts</span>
                <input type="checkbox" checked={prefs.auto_send_transcripts}
                  onChange={(e) => updatePrefs({ auto_send_transcripts: e.target.checked })} />
              </div>
              <p className="text-xs text-muted-foreground">Preferences save automatically to your account.</p>
            </div>
          )}
          ...
          {panel === "chat" && (
            <>
              <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
                {messages.length === 0 ? (
                  <div className="space-y-3">
                    <p className="text-sm text-muted-foreground">Ask Atlas about your operations, revenue, claims, or team. Conversations save automatically.</p>
                    {SUGGESTIONS.map((s) => (
                      <button
                        key={s}
                  ...
                  messages.map((m) => {
                    ...
                    return (
                      <div key={m.id} className={cn("text-sm", m.role === "user" ? "text-right" : "text-left")}>
                        ...
                        >
                          {text || (m.role === "assistant" && busy ? <Loader2 className="h-3 w-3 animate-spin" /> : null)}
                        </div>
                      </div>
                    );
                  })
              ...
              <form onSubmit={submit} className="p-3 border-t border-border flex items-center gap-2">
                ...
                />
                {busy ? (
                  <Button type="button" size="icon" variant="secondary" onClick={stop} className="h-9 w-9 shrink-0">
                    <X className="h-4 w-4" />
                  </Button>
                ) : (
