...
export function CommandPalette({
    ...
    const t = setTimeout(async () => {
      ...
      const [claims, customers, adjusters, docs] = await Promise.all([
        db
          ...
          .or(`claim_number.ilike.${like},description.ilike.${like}`)
          .limit(4),
        db
          .from("customers")
      ...
      const merged: Hit[] = [
        ...(
          (claims.data ?? []) as { id: string; claim_number: string; description: string | null }[]
        ).map((c) => ({
          kind: "claim" as const,
          id: c.id,
        ...
        ...((customers.data ?? []) as { id: string; name: string; email: string | null }[]).map(
          (c) => ({
        ...
        ...((adjusters.data ?? []) as { id: string; name: string; email: string | null }[]).map(
          (a) => ({
            kind: "adjuster" as const,
            id: a.id,
            label: a.name,
      ...
      if (q.length >= 4) {
        try {
          ...
          /* embeddings may be pending — silent */
        }
      } else {
        setSemantic([]);
      }
  ...
  function ask(prompt: string) {
    onOpenChange(false);
    setQuery("");
    window.dispatchEvent(new CustomEvent("atlas:ask", { detail: { prompt } }));
  }
  function openHit(kind: string, id: string) {
    onOpenChange(false);
    setQuery("");
    if (kind === "claims" || kind === "claim")
      navigate({ to: "/app/claims/$claimId", params: { claimId: id } });
    else if (kind === "supplements")
      navigate({ to: "/app/supplements/$supplementId", params: { supplementId: id } });
      ...
      if (intent.intent === "ask") {
        ...
        if (intent.entity_type === "claims") {
          const { data } = await db
            .from("claims")
            .select("id")
            .ilike("claim_number", intent.entity_query)
            .limit(1)
            .maybeSingle();
        ...
        const res = await runSemantic({
          data: {
            query: intent.entity_query,
            types: [intent.entity_type as SemanticHit["entity_type"]],
            limit: 1,
          },
        });
      ...
      <CommandList>
        ...
        {hits.length > 0 && (
          <CommandGroup heading="Matches">
            {hits.map((h) => (
              <CommandItem
                key={`${h.kind}:${h.id}`}
                onSelect={() => openHit(h.kind, h.id)}
                value={`match-${h.kind}-${h.id}-${h.label}`}
              >
                <Search className="mr-2 h-4 w-4 text-muted-foreground" />
                <span className="mr-2 text-[10px] uppercase text-muted-foreground">{h.kind}</span>
                <span className="truncate">{h.label}</span>
                {h.sub && (
                  <span className="ml-2 text-xs text-muted-foreground truncate">— {h.sub}</span>
                )}
              </CommandItem>
            ))}
          </CommandGroup>
        ...
        {semantic.length > 0 && (
          <CommandGroup heading="Related by meaning">
            {semantic.map((h) => (
              <CommandItem
                key={`sem-${h.entity_type}:${h.entity_id}`}
                onSelect={() => openHit(h.entity_type, h.entity_id)}
                value={`sem-${h.entity_type}-${h.entity_id}-${h.label}`}
              >
