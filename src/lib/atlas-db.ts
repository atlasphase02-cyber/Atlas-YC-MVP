...
export interface Adjuster {
  id: string; owner_id: string; carrier_id: string | null; name: string;
  email: string | null; phone: string | null;
  avg_response_hours: number | null; notes: string | null;
  created_at: string; updated_at: string;
}
...
export interface Claim {
  id: string; owner_id: string; claim_number: string;
  customer_id: string | null; carrier_id: string | null; adjuster_id: string | null;
  status: ClaimStatus; amount_cents: number;
  loss_date: string | null; description: string | null;
  archived_at?: string | null;
  created_at: string; updated_at: string;
}
export interface Supplement {
  id: string; owner_id: string; claim_id: string;
  status: SupplementStatus; summary: string | null;
  total_cents: number; ai_confidence: number | null;
  ai_summary: string | null;
  ai_recommendations: string[];
  created_at: string; updated_at: string;
}
export interface SupplementItem {
  id: string; owner_id: string; supplement_id: string;
  description: string; quantity: number; unit_price_cents: number;
  ai_suggested: boolean; ai_confidence: number | null; ai_reason: string | null;
  created_at: string;
}
export interface AppDocument {
  id: string; owner_id: string;
  claim_id: string | null; customer_id?: string | null;
  folder: string; name: string;
  storage_path: string | null; mime_type: string | null;
  size_bytes: number | null; tags: string[];
  version?: number; ocr_status?: string;
  created_at: string;
}
export interface DocumentVersion {
  id: string; owner_id: string; document_id: string;
  version: number; storage_path: string;
  size_bytes: number | null; mime_type: string | null;
  created_at: string;
}
export interface Photo {
  id: string; owner_id: string; claim_id: string | null;
...
export interface ClaimEvent {
  id: string; owner_id: string; claim_id: string;
  kind: string; detail: string | null; created_at: string;
}
export interface Appointment {
  id: string; owner_id: string; claim_id: string | null;
  kind: AppointmentKind; title: string; who: string | null;
...
export interface AppNotification {
  ...
  tone: NotificationTone; read_at: string | null;
  link_to?: string | null; created_at: string;
}
export interface NotificationPreferences {
  user_id: string;
  email_enabled: boolean; in_app_enabled: boolean;
  claim_updates: boolean; supplement_updates: boolean;
...
export interface ChatMessage {
  id: string; owner_id: string; conversation_id: string;
  role: MessageRole; content: string;
  parts: unknown | null; created_at: string;
}
export type VoiceMode = "tap" | "ptt" | "hands_free";
export interface VoicePreferences {
  user_id: string; voice_name: string | null;
  rate: number; pitch: number; muted: boolean;
  mode: VoiceMode; auto_send_transcripts: boolean;
  updated_at: string;
}
export type InterviewStatus = "active" | "completed" | "abandoned";
export interface InterviewTurn { role: "user" | "assistant"; content: string; at: string }
export interface InterviewActionItem { title: string; owner?: string | null; due?: string | null }
export interface Interview {
  id: string; owner_id: string;
  claim_id: string | null; customer_id: string | null;
  title: string; status: InterviewStatus;
...
export async function logClaimEvent(claimId: string, kind: string, detail?: string) {
  const uid = await currentUserId();
  if (!uid) return;
  await db.from("claim_events").insert({ owner_id: uid, claim_id: claimId, kind, detail: detail ?? null });
}
